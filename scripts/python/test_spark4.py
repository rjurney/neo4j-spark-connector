...
class SparkTest(unittest.TestCase):
    ...
    def tearDown(self):
        with self.neo4j_driver.session() as session:
            ...
            for constraint in session.run("SHOW CONSTRAINTS YIELD name RETURN name").data():
                session.run(f"DROP CONSTRAINT `{constraint['name']}` IF EXISTS").consume()
            for index in session.run("SHOW INDEXES YIELD name, type WHERE type <> 'LOOKUP' RETURN name").data():
                session.run(f"DROP INDEX `{index['name']}` IF EXISTS").consume()
